...
def parse_args():
    ...
    parser.add_argument("--half", type=parse_bool, default=False, help="Use half neighbor list for local-local pairs.")
    parser.add_argument("--threads", type=int, default=1, help="Number of CPU threads per MPI rank.")
    parser.add_argument("--steps", type=int, default=0, help="Number of MD steps after the initial prepare stage.")
    ...
    parser.add_argument("--kbuf", type=float, default=10.5 / 8.5, help="Neighbor-list buffer factor.")
...
def main():
    ...
    atoms = Atoms(0)
    if rank == 0:
        data_path = Path(__file__).with_name("LMP_BALL_130k_Ar")
    ...
    xmin, xmax = -9.6934310913, 21.8065689087
    ymin, ymax = -9.6934310913, 21.8065689087
    zmin, zmax = -9.6934310913, 21.8065689087
    ...
    threads = Threads(args.threads, comm)

    R = 10.365
    xc = yc = zc = (xmax + xmin) / 2.0
    ...
    if args.write_log:
        if rank == 0:
            print(
                ...
                % (args.half, args.threads, size, args.steps, max_elapsed, total_rebuilds),
                flush=True,
            )
            logfile.close()
